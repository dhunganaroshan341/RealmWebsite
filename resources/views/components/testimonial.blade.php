            ...
            <div class="testimonial-list">
                <div class="card p-3">
                    <div class="d-flex flex-row align-items-center">
                        <img id="clientImage" src="{{ $testimonials[0]->image ?? '' }}" width="50" class="rounded-circle">
                        <div class="d-flex flex-column ml-2">
                            <span id="clientName" class="font-weight-normal">{{ $testimonials[0]->customer_name ?? 'Milton Austin' }}</span>
                            <span id="clientPosition">{{ $testimonials[0]->customer_position ?? 'Sales Manager, Stack' }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-8">
            <div class="testimonial-content">
                <div id="testimonialText">
                    <h4>{{ $testimonials[0]->testimonial_title ?? 'It was a great experience' }}</h4>
                    <div class="ratings">
                        @for($i = 0; $i < $testimonials[0]->rating; $i++)
                            <i class="fa fa-star"></i>
                        @endfor
                    </div>
                    <p>{{ $testimonials[0]->testimonial_text ?? 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.' }}</p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        let totalTestimonials = {{ count($testimonials) }};  // Total number of testimonials

        // Function to update the testimonial content
        function updateTestimonial(index) {
            // Update the client name, position, image, and testimonial content
            let testimonial = @json($testimonials);  // Convert the PHP array into a JS array
